...
# Make sure to import BossB52
if "BossB52" not in content:
    content = content.replace("ItemDrop\n", "ItemDrop, BossB52\n")
    ...
    while running:
        if app_state == "MAIN_MENU":
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_1: current_level = 1
                        ...
                        if current_level == 1:
                            # 8 địch, chậm
                            for _ in range(2):
                                enemies.append(DFSEnemy(get_random_pixel_offset(600, 1500)[0], get_random_pixel_offset(600, 1500)[1]))
                                enemies.append(BFSEnemy(get_random_pixel_offset(600, 1500)[0], get_random_pixel_offset(600, 1500)[1]))
                                enemies.append(AStarEnemy(get_random_pixel_offset(600, 1500)[0], get_random_pixel_offset(600, 1500)[1]))
                                enemies.append(HeuristicEnemy(get_random_pixel_offset(600, 1500)[0], get_random_pixel_offset(600, 1500)[1]))
                            for e in enemies: e.max_speed *= 0.7 # Giảm tốc độ
                            pygame.time.set_timer(AI_UPDATE_EVENT, 500)
                            ...
                            for _ in range(4):
                                ...
                                enemies.append(AStarEnemy(get_random_pixel_offset(600, 1500)[0], get_random_pixel_offset(600, 1500)[1]))
                                enemies.append(HeuristicEnemy(get_random_pixel_offset(600, 1500)[0], get_random_pixel_offset(600, 1500)[1]))
                            pygame.time.set_timer(AI_UPDATE_EVENT, 200) # AI siêu nhạy
            ...
            if game_state == "PLAYING" and not show_visualizer:
                ...
                # Va chạm người chơi với địch (Gây sát thương đâm)
                for enemy in enemies[:]:
                ...
                if len(enemies) == 0:
                    game_state = "WIN"
                    global_profile.money += current_level * 100 # Thưởng qua màn
